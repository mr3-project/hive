...
public class Optimizer {
  ...
  public void initialize(HiveConf hiveConf) {
    ...
    boolean isMR3ExecEngine = HiveConf.getVar(hiveConf, HiveConf.ConfVars.HIVE_EXECUTION_ENGINE).equals("mr3");
    ...
    if (HiveConf.getBoolVar(hiveConf, HiveConf.ConfVars.HIVECOUNTDISTINCTOPTIMIZER)
        && (HiveConf.getBoolVar(hiveConf, HiveConf.ConfVars.HIVE_IN_TEST) || isTezExecEngine || isMR3ExecEngine)) {
    ...
    if (HiveConf.getBoolVar(hiveConf, HiveConf.ConfVars.HIVE_OPTIMIZE_SKEWJOIN_COMPILETIME)) {
      if (!isTezExecEngine && !isMR3ExecEngine) {
    ...
    if ((HiveConf.getBoolVar(hiveConf, HiveConf.ConfVars.HIVEOPTBUCKETMAPJOIN))
      && !isTezExecEngine && !isMR3ExecEngine && !isSparkExecEngine) {
    ...
    if (pctx.getContext().isExplainSkipExecution() && !isTezExecEngine && !isMR3ExecEngine && !isSparkExecEngine) {
